//
// Created by Lenovo on 2022/5/20.
//
#include <cstdio>
#include <iostream>
#include <algorithm>
...
void Save_quantization_data(const char *filename) {
    ...
    for (unsigned i = 0; i < sub_count; i++) {
        for (unsigned j = 0; j < sub_dim; j++) {
            out.write((char *) &quant_vector[i][j], sizeof(short));
        }
    }
    out.close();
}
...
void Load_quantization_data(const char *filename) {
    ...
    for (unsigned i = 0; i < sub_count; i++) {
        ...
        for (unsigned j = 0; j < sub_dim; j++) {
            in.read((char *) &quant_vector[i][j], sizeof(short));
        }
    }
    in.close();
}
...
int main(int argc, char **argv) {
    ...
    if (isFileExists_ifstream("code_vec64.nsg")) {
        ...
        Save_code_vec("code_vec64_4096.nsg");
    }
...
//    0 16 35 5 32 31 14 10
    if (isFileExists_ifstream("quanti_vec64.nsg")) {
        ...
        Save_quantization_data("quanti_vec64.nsg");
    }
    // TEST loss
